...
}>({ allowed: {}, can: () => false, canPath: () => false, isDeveloper: false, loading: true, error: null, refresh: () => {}, home: "/profile" });
...
export function AccessProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (user && query.data && (user.role !== query.data.role || (user.group?.id ?? null) !== query.data.group_id || Boolean(user.is_developer) !== (query.data.capabilities?.manage_sessions === true))) retryRestore();
