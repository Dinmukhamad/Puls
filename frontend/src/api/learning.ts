...
export interface LearningContent {
  ...
  is_required: boolean; deadline: string | null; allow_back: boolean; pass_percent: number;
  xp_reward: number; coins_reward: number; revision: number; step_count: number; steps?: LearningStep[];
...
export interface LearningAttempt {
  ...
  score: number | null; correct: number | null; awarded_xp: number; awarded_coins: number; finished_at: string | null;
}
export interface LearningResult {
  id: number; user_id: number; full_name: string; content_id: number; title: string; kind: LearningKind;
  state: string; answered: number; total: number; score: number | null; awarded_xp: number;
