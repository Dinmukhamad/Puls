...
export interface WheelSegment { title: string; weight: number; coins: number }
...
export interface Raffle { id: number; title: string; description: string; prize: string; closes_at: string; status: "draft" | "published" | "closed"; coins_reward: number; participants: number; entered: boolean; winner_name: string | null; won: boolean; drawn_at: string | null; accepting_entries: boolean }
export type RaffleInput = Pick<Raffle,"title" | "description" | "prize" | "closes_at" | "coins_reward"> & { status: "draft" | "published" };
export const games = {
