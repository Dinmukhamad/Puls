  ...
  return <div className="city-page">
    ...
    <div className="city-layout"><CityMap mascot={mascot} districts={data.districts} missions={data.missions} selected={selected.id} onSelect={selectDistrict} progressKey={!data.inspecting && !data.preview ? `city-levels:${data.user_id}` : undefined} />
      <aside className="city-mission" aria-live="polite">
        ...
        {selected.soon ? <><h2>{selected.id === "dispatch" ? "Держать город в движении" : "Новый район. Новые возможности."}</h2><p className="city-copy">{selected.id === "dispatch" ? "Этот район откроется вместе с учебной диспетчерской." : "Миссии появятся после добавления Oktell и подготовки учебных сценариев."}</p><div className="city-guide"><PulsarFace /><p>А пока продолжим развивать автопарк и CRM-центр. Твой прогресс сохранится.</p></div><button className="city-action" disabled>Район строится</button></> : mission && <>
