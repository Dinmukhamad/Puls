...
import { DriverProfileViews } from "./DriverProfileViews";
...
function ShiftReview({ shift, state, act, go, busy, hint, onHint }: ShiftViewProps & { hint: boolean; onHint: () => void }) {
  ...
  return <><DCard title={c.title}><strong className="ds-big">{result ? result.score === null ? "Свободная практика" : `${result.score}/100` : `${shift.data.completed}/${c.target_orders} заказов`}</strong><p>{result ? "Смена завершена. Ниже — результат и путь выполнения каждого пункта." : `Задание: выберите парк ${state.parks.find(x => x.id === c.required_park)?.name ?? c.required_park}, выполните ${c.target_orders} заказов, проверьте рейтинг и приоритет.`}</p>{!result && <p>{c.require_photo && "Пройдите фотоконтроль. "}{c.route_event && "Обработайте изменение адреса в первом заказе. "}{c.require_support && "Разберите проблему с выплатой: сначала детали операции, затем поддержка в Telegram. "}{c.require_documents && "Выберите ЭДО и подпишите учебный акт."}</p>}</DCard>
    {result ? <><DCard title="Разбор задания">{result.checks.map(x => <div className="ds-check" key={x.key}><b className={x.done ? "ds-success" : ""}>{x.done ? "✓" : "○"}</b><div><strong>{x.title}</strong><p>{x.path}</p></div><span>{x.done ? x.weight : 0}/{x.weight}</span></div>)}</DCard><DCard title="Как прошла смена"><DRow title="Заказы" value={`${result.orders}/${result.target}`} /><DRow title="Время" value={`${Math.floor(result.seconds / 60)} мин ${result.seconds % 60} с`} /><DRow title="Ошибки" value={result.errors} /><DRow title="Подсказки" value={result.hints} /><DRow title="Снято баллов" value={result.penalties} /><DRow title="Лучший результат" value={state.shift_best === null ? "—" : `${state.shift_best}/100`} /></DCard><Link className="driver-primary" to="/training?kind=simulator">Новая смена в обучении</Link></> : <DCard title="Управление сменой"><button className="driver-secondary" disabled={busy} onClick={onHint}>Показать путь выполнения{shift.mode === "assessment" ? " · −1 балл" : ""}</button>{hint && <p>Профиль → Фотоконтроль → Заказы → завершение и оплата. После задержки: Деньги → нужная выплата → Поддержка → Telegram → снова операция. Профиль → Рейтинг, Приоритет, Юридические документы → ЭДО → Подписать.</p>}<button className="driver-primary" onClick={() => go("orders")}>Продолжить смену</button>{confirm ? <><p>Завершить с текущим результатом? Невыполненные пункты останутся в разборе.</p><button className="driver-primary" disabled={busy} onClick={() => act("finish")}>Подтвердить завершение смены</button><button className="driver-secondary" onClick={() => setConfirm(false)}>Продолжить работу</button></> : <button className="driver-secondary" onClick={() => setConfirm(true)}>Завершить и посмотреть результат</button>}</DCard>}
