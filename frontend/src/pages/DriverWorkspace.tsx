...
import { ShiftRing } from "./DriverMotion";
...
export function DriverWorkspace({ state, fullName, busy: parentBusy, error: parentError, failed, create, orderAction, switchPark, refresh }: Props) {
  ...
  return <div className={`driver-app driver-workspace${light ? " ds-light" : ""}${theming ? " ds-theming" : ""}${prefs.hide_income ? " ds-hide-income" : ""}`}>
    <header className="driver-header"><button className="driver-exit" onClick={() => setExit(true)}>‹ Puls</button><button className="ds-shift-name" onClick={() => go("shift-result")}><ShiftRing done={shift.data.completed} total={shift.config.target_orders} /><span>{shift.is_preview ? "Тестовая проверка" : shift.mode === "free" ? "Свободный режим" : "Учебная смена"}</span><b>{shift.data.completed}/{shift.config.target_orders}</b></button><span className="driver-environment">Учебный</span></header>
