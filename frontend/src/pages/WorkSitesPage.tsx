...
export function WorkSitesPage() {
  ...
  const catalog = useQuery({ queryKey: ["crm-catalog"], queryFn: crm.catalog });
  ...
  function go(query: string, force = false) {
    ...
    setDirty(false); setParams(query); setNotice("");
  }
  ...
  return <div className="work-sites" ref={shell} data-guide={guide === null ? "" : GUIDE_STEPS[guide].target}>
    <header className="work-sites-heading"><button onClick={exit}>← Обучение</button><h1>Рабочие сайты</h1><span className="work-sites-training">Учебная среда</span><button className="work-sites-guide-button" onClick={() => setGuide(0)}>✦ Помощь Пульсара</button></header>
    ...
    {guide !== null && <PulsarGuide step={guide} onClose={hideGuide} onPrevious={() => setGuide(s => Math.max(0, (s ?? 1) - 1))} onNext={() => { if (guide === GUIDE_STEPS.length - 1) hideGuide(); else { if (guide === 0 && view !== "create") create(); setGuide(guide + 1); } }} />}
