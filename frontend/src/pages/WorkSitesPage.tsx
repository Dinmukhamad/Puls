...
export function WorkSitesPage() {
  ...
  const seenKey = (tour: CoachTourId) => tour === "appeals" ? `crm-guide:${user?.id}` : `crm-guide-drivers:${user?.id}`;
  ...
  const endCoach = useCallback((finished: boolean) => { setCoach(current => { if (current) try { localStorage.setItem(seenKey(current), finished ? "done" : "closed"); } catch { /* Optional preference. */ } return null; }); }, [user?.id]); // eslint-disable-line react-hooks/exhaustive-deps
  const sectionTour: CoachTourId | null = view === "drivers" && !driverId ? "drivers" : (view === "list" && !id) || view === "create" ? "appeals" : null;
  useEffect(() => {
    if (!catalog.data || !sectionTour || coach) return;
    let seen = true; try { seen = !!localStorage.getItem(seenKey(sectionTour)); } catch { /* Without storage the tour stays manual. */ }
    if (!seen) { const timer = window.setTimeout(() => setCoach(sectionTour), 600); return () => window.clearTimeout(timer); }
  ...
  return <div className={`work-sites${coach ? " is-coaching" : ""}`} ref={shell}>
    ...
    <PulsarGuide catalog={catalog.data} screen={pulsarScreen} categoryIds={selection} step={null} editor={instructionEditor} openRequest={helpRequest} onStart={() => startCoach(view === "drivers" ? "drivers" : "appeals")} onClose={() => {}} onPrevious={() => {}} onNext={() => {}} />
    {coach && <PulsarCoach key={coach} steps={COACH_TOURS[coach]} onClose={endCoach} />}
  </div>;
}
