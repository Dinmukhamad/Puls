...
function AttemptPlayer({ id }: { id: number }) {
  ...
  return <div className="learning-player stack"><Link to="/training" className="learning-back">‹ Обучение</Link><header><Badge tone="xp">{attempt.content.kind === "test" ? "Проверка знаний" : attempt.content.world}</Badge><h1 className="page-title">{attempt.content.title}</h1></header>
    <div className="stack stack--tight"><p className="secondary">{attempt.content.kind === "test" ? "Вопрос" : "Шаг"} {step + 1} из {attempt.content.steps.length}</p><Progress value={step / attempt.content.steps.length} tone="xp" label="Прогресс задания" /></div>
...
export function LearningResult({ attempt }: { attempt: LearningAttempt }) {
  ...
  return <div className="learning-player stack"><Link className="learning-back" to="/training">‹ Обучение</Link><Card className="learning-result"><span className="training-eyebrow">{attempt.content.title}</span><h1>Задание завершено</h1><div className="learning-result__score">{attempt.score}%</div><Badge tone={passed ? "success" : "warning"}>{passed ? "✓ Пройдено" : "Попробуйте ещё раз"}</Badge><p>{attempt.correct} из {attempt.content.steps.length} правильных решений</p><div className="row learning-result__rewards">{attempt.awarded_xp > 0 && <Badge tone="xp">+{attempt.awarded_xp} XP</Badge>}{attempt.awarded_coins > 0 && <Badge tone="coin">+{attempt.awarded_coins} коинов</Badge>}</div><p className="secondary small">{passed ? "Награда за этот материал выдаётся один раз. Повторение помогает закрепить знания." : `Для прохождения необходимо ${attempt.content.pass_percent}% правильных ответов.`}</p></Card>
