...
function DriverDetails({ driver: d, ...shared }: Shared & { driver: TrainingDriver }) {
  ...
  return <div className="drv-details">
    <section className="crm-panel drv-card">
...
function Field({ label, error, children, wide }: { label: string; error?: string; children: ReactNode; wide?: boolean }) {
  return <label className={`drv-field${error ? " has-error" : ""}${wide ? " is-wide" : ""}`}><span>{label}</span><div>{children}{error && <small role="alert">{error}</small>}</div></label>;
}
...
function CarEditor({ driver: d, update, open, notify }: Shared & { driver: TrainingDriver }) {
  ...
  return <section className="crm-panel drv-form"><div className="crm-panel-heading"><div><h2>🚗 Редактор автомобиля водителя: {d.account}</h2><p>{driverName(d)} · сейчас {carTitle(d.car)}, {d.car.plate}</p></div><button className="crm-link" onClick={() => open(d.id, "details")}>← К карточке водителя</button></div>
    <form className="drv-form-body" noValidate onSubmit={e => { e.preventDefault(); setTried(true); if (Object.keys(validateCar(car, strict)).length) return; const replaced = car.plate !== d.car.plate; update(d.id, x => changeCar(x, car)); open(d.id, "details"); notify(replaced ? `Автомобиль ${driverName(d)} изменён на ${carTitle(car)} ${car.plate}. Скажите водителю пройти фотоконтроль автомобиля и техпаспорта.` : "Данные автомобиля сохранены."); }}>
      <h3 className="drv-form-title">Выбор автомобиля</h3>
      <div className="drv-toggle" role="group" aria-label="Автомобиль"><button type="button" aria-pressed={mode === "current"} onClick={() => switchMode("current")}>Существующий</button><button type="button" aria-pressed={mode === "new"} onClick={() => switchMode("new")}>Новый</button></div>
      ...
      <div className="drv-grid">
        ...
        <Field label="Госномер *" error={errors.plate}><input value={car.plate} placeholder="Например, 803ASD02" onChange={e => set("plate", e.target.value.toUpperCase().replace(/\s/g, ""))} /></Field>
        <Field label="Марка *" error={errors.brand}><select value={car.brand} onChange={e => setCar(c => ({ ...c, brand: e.target.value, model: "" }))}><option value="">Выберите марку</option>{Object.keys(CAR_BRANDS).map(b => <option key={b}>{b}</option>)}</select></Field>
        <Field label="VIN"><input value={car.vin} onChange={e => set("vin", e.target.value.toUpperCase())} /></Field>
        <Field label="Модель *" error={errors.model}><select value={car.model} disabled={!car.brand} onChange={e => set("model", e.target.value)}><option value="">Выберите модель</option>{(CAR_BRANDS[car.brand] ?? []).map(m => <option key={m}>{m}</option>)}</select></Field>
        <Field label="Номер кузова"><input value={car.body} onChange={e => set("body", e.target.value)} /></Field>
        <Field label="Цвет *" error={errors.color}><select value={car.color} onChange={e => set("color", e.target.value)}><option value="">Выберите цвет</option>{CAR_COLORS.map(c => <option key={c}>{c}</option>)}</select></Field>
        <Field label="СТС"><input value={car.sts} onChange={e => set("sts", e.target.value)} /></Field>
        <Field label="Год *" error={errors.year}><select value={car.year || ""} onChange={e => set("year", Number(e.target.value))}><option value="">Выберите год</option>{years.map(y => <option key={y}>{y}</option>)}</select></Field>
        <Field label={strict ? "Позывной *" : "Позывной"} error={errors.callsign}><span className="drv-inline"><input value={car.callsign} onChange={e => set("callsign", e.target.value)} /><button type="button" className="crm-secondary" disabled={!car.plate} onClick={() => set("callsign", car.plate)}>⧉ Скопировать госномер</button></span></Field>
      ...
      <div className="drv-grid">
        ...
        <div><h3 className="drv-form-title">Параметры и тарифы</h3><Field label="Вид топлива"><select value={car.fuel} onChange={e => set("fuel", e.target.value)}>{["Бензин", "Газ", "Гибрид", "Электро", "Дизель"].map(f => <option key={f}>{f}</option>)}</select></Field>
          <fieldset className="drv-tariffs"><legend>Тариф / Класс</legend>{TARIFFS.map(t => <label key={t} className={car.tariffs.includes(t) ? "is-on" : ""}><input type="checkbox" checked={car.tariffs.includes(t)} onChange={() => set("tariffs", car.tariffs.includes(t) ? car.tariffs.filter(x => x !== t) : [...car.tariffs, t])} />{t}</label>)}</fieldset></div>
      </div>
      <div className="drv-form-footer"><button className="crm-primary">✓ Сохранить</button><button type="button" className="crm-secondary" onClick={() => open(d.id, "details")}>Отмена</button>{mode === "new" && <span>После смены автомобиля водителю нужно пройти фотоконтроль автомобиля и техпаспорта.</span>}</div>
    </form></section>;
}
