...
export function PulsarCoach({ steps, onClose }: { steps: CoachStep[]; onClose: (finished: boolean) => void }) {
  ...
  const next = () => { if (last) onClose(true); else setIndex(i => i + 1); };
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (step.skipWhen && document.querySelector(step.skipWhen) && index < steps.length - 1) { setIndex(i => i + 1); return; }
  ...
  return <div className="coach" data-flying={flying} data-missing={missing}>
    ...
    <div ref={bubbleRef} className="coach-bubble" role="dialog" aria-live="polite" aria-label={`Пульсар: ${step.title}`}>
      <div className="coach-bubble-top"><span className="coach-count">{index + 1} / {steps.length}</span><button className="coach-close" aria-label="Закрыть подсказки" onClick={() => onClose(false)}>✕</button></div>
      <strong>{step.title}</strong>
      <p>{missing ? "Этого элемента сейчас нет на экране. Нажми «Дальше», и я покажу следующее." : step.text}</p>
      <div className="coach-dots" aria-hidden="true">{steps.map((_, i) => <span key={i} className={i < index ? "is-done" : i === index ? "is-current" : ""} />)}</div>
      <div className="coach-actions">
        <button className="coach-back" disabled={index === 0} onClick={() => setIndex(i => Math.max(0, i - 1))}>← Назад</button>
        <button className="coach-next" autoFocus onClick={next}>{last ? "Готово ✓" : step.advanceWhen && !missing ? "Пропустить →" : "Дальше →"}</button>
      </div>
    </div>
  </div>;
}
