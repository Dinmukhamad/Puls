...
export function CrmAppealForm({ catalog, initial, onSaved, onDirty, onCategoryChange, onRequestHelp }: { onCategoryChange: (ids: string[]) => void; onRequestHelp: () => void; catalog: CrmCatalog; initial?: CrmAppeal; onSaved: (appeal: CrmAppeal, duplicate: boolean) => void; onDirty: (dirty: boolean) => void }) {
  ...
  const input = (key: "phone" | "license_number" | "driver_id", label: string, required: boolean) => <label className="crm-field"><span>{label}{required && " *"}</span><input required={required} value={form[key]} maxLength={key === "phone" ? 40 : 80} type={key === "phone" ? "tel" : "text"} onChange={e => update(key, e.target.value)} placeholder={label} /></label>;
  return <section className="crm-panel crm-form-panel"><div className="crm-panel-heading"><h2>＋ Создать обращение</h2><span>* Обязательные поля</span></div>
    <form onSubmit={e => { e.preventDefault(); save.mutate(); }} onPaste={e => { const images = Array.from(e.clipboardData.files).filter(f => f.type.startsWith("image/")); if (images.length) { e.preventDefault(); attach(images); } }}>
      <fieldset disabled={save.isPending} className="crm-fields">
        <div data-tour="contact"><label className="crm-field"><span>Звонок/Чат</span><select value={form.channel} onChange={e => update("channel", e.target.value)}><option>Звонок</option><option>Чат</option></select></label>
        ...
        <label className="crm-field"><span>Таксопарк *</span><select required value={form.park} onChange={e => { update("park", e.target.value); update("city", ""); }}><option value="">Выберите таксопарк</option>{catalog.parks.map(park => <option key={park}>{park}</option>)}</select></label>
        ...
        {!!form.category_ids.length && <div className="crm-context-help"><PulsarFace /><div><button className="crm-help-link" type="button" onClick={onRequestHelp}>✦ {instruction?.title ?? "Инструкция по обращению"} — открыть у Пульсара</button><p>{instruction?.body}</p></div></div>}
        ...
        <div className="crm-form-options" data-tour="save"><label><input type="checkbox" checked={form.is_ticket} onChange={e => update("is_ticket", e.target.checked)} /> Формировать тикет</label><small>Сохранить запрос для учебной обработки отделом</small><label><input type="checkbox" checked={duplicate} onChange={e => setDuplicate(e.target.checked)} /> Дублировать обращение</label><small>После сохранения открыть копию для следующего обращения</small></div>
      </fieldset>
      <div className="crm-form-footer"><button className="crm-primary" disabled={save.isPending}>{save.isPending ? "Сохранение…" : "↓ Сохранить"}</button><span>Обращение будет видно всем участникам</span>{save.isError && <p className="crm-error" role="alert">{save.error.message}</p>}</div>
    </form>
  </section>;
}
