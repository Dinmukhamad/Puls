...
export function SummaryPage() {
  ...
  return <div className="stack analytics-page">
    <header className="page-head"><div><h1 className="page-title">Сводка</h1><p className="page-subtitle">Состояние команды, результаты и действия на сегодня.</p></div><Link className="btn btn--primary" to="/admin/periods">Расчёт периода</Link></header>
    ...
    {data && !report.data.isError && <>
      ...
      <div className="analytics-summary-grid"><Card title="Требует внимания" subtitle={attention.length ? "Ближайшие действия для команды" : "По доступным данным новых сигналов нет"}>{attention.length ? <div className="analytics-attention-list">{attention.map((item) => <Link className="analytics-attention" key={item.href} to={item.href}><span className="analytics-attention__count">{item.count}</span><span><strong>{item.title}</strong><span>{item.text}</span></span><span aria-hidden="true">→</span></Link>)}</div> : <EmptyState title="Всё спокойно" hint="Здесь появятся пропуски данных, результаты ниже цели и новые заявки." />}</Card>
        <Card title="Расчёт периода" subtitle={data.week?.label ?? "Периодов пока нет"}><div className="stack">{data.week ? <><Badge tone={data.week.status === "closed" ? "success" : "warning"}>{WEEK_STATUS_LABELS[data.week.status]}</Badge><p className="analytics-muted">{data.week.status === "closed" ? "Результаты опубликованы. Начисления отражены в кошельках операторов." : data.week.status === "calculated" ? "Расчёт готов к проверке и публикации." : "Загрузите показатели, проверьте данные и выполните расчёт."}</p><Link className="btn btn--secondary" to={`/admin/periods?week=${data.week.id}`}>Открыть период</Link></> : <EmptyState title="Начните с первой недели" action={<Link className="btn btn--primary" to="/admin/periods">Создать период</Link>} />}</div></Card></div>
      {metric && <Card title={`Команда · ${metric.title}`} subtitle="Среднее по наблюдаемым значениям за восемь недель" action={<Link className="btn btn--plain" to={`/analytics${data.week ? `?week_id=${data.week.id}` : ""}`}>Вся аналитика →</Link>}><div className="analytics-summary-metric"><strong>{points(metric.value)} {metric.unit}</strong><MetricDelta value={metric.delta} improved={metric.improved} unit={metric.unit} /></div><Chart compact title={metric.title} labels={data.trend.map((item) => item.label)} series={[{ id: "team", name: "Команда", values: data.trend.map((item) => item.value) }]} target={metric.target} unit={metric.unit} /></Card>}
