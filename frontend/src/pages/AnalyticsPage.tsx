...
export function ReportFilters({ report, metric = true }: { report: ReturnType<typeof useAnalyticsReport>; metric?: boolean }) {
  ...
  const fields = <>
    <label className="field"><span>Период</span><select className="input" value={report.filters.week_id ?? ""} onChange={(event) => report.update({ week_id: event.target.value, page: undefined })}>
    ...
    <label className="field"><span>Группа</span><select className="input" value={report.filters.group_id ?? ""} onChange={(event) => report.update({ group_id: event.target.value, operators: undefined, group_compare: undefined, page: undefined })}>
      <option value="">Все доступные</option>{(report.groups.data ?? []).map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}
    </select></label>
    {metric && <label className="field"><span>Показатель</span><select className="input" value={report.data.data?.metric_code ?? report.filters.metric_code ?? ""} onChange={(event) => report.update({ metric: event.target.value, page: undefined })}>
...
export function AnalyticsPage() {
  ...
  const comparisonControls = <>
    ...
    {comparisonMode === "groups" ? <fieldset className="analytics-selection"><legend>До 5 групп</legend>{(value?.groups ?? []).map((group) => <label key={group.id}><input type="checkbox" checked={selectedGroups.includes(group.id)} disabled={selectedGroups.length >= 5 && !selectedGroups.includes(group.id)} onChange={() => update({ group_compare: (selectedGroups.includes(group.id) ? selectedGroups.filter((id) => id !== group.id) : [...selectedGroups, group.id]).join(",") || "none" })} />{group.name}</label>)}</fieldset> : <div className="analytics-selection">
      <label className="field"><span>Добавить оператора · до 5</span><select className="input" value="" disabled={selectedOperators.length >= 5} onChange={(event) => { if (event.target.value) update({ operators: [...selectedOperators, Number(event.target.value)].join(",") }); }}><option value="">Выберите сотрудника</option>{(value?.operators ?? []).filter((row) => !selectedOperators.includes(row.user_id)).map((row) => <option key={row.user_id} value={row.user_id}>{row.full_name}</option>)}</select></label>
    ...
    {value && !data.isError && <>
      {!value.week ? <EmptyState title="Периодов пока нет" hint="Загрузите показатели и выполните расчёт периода." action={<Link hideWhenDenied className="btn btn--primary" to="/admin/periods">Перейти к расчёту</Link>} /> : !metric ? <EmptyState title="Показатели не настроены" hint="Добавьте определения показателей в настройках системы." /> : <>
        ...
        {tab !== "summary" && <>
          ...
          <div className="analytics-row-filters"><label className="field"><span>Найти оператора</span><input className="input" type="search" placeholder="Имя сотрудника" value={search} onChange={(event) => update({ search: event.target.value, page: undefined })} /></label><label className="field"><span>Показать</span><select className="input" value={status} onChange={(event) => update({ status: event.target.value, page: undefined })}><option value="all">Всех</option><option value="below">Ниже цели</option><option value="missing">Без данных</option></select></label></div>
