...
export function AccessAdminPage() {
  ...
  return <div className="stack access-page">
    ...
    <Card title="Как работают правила"><p>Приоритет: аккаунт → группа → роль → все сотрудники → настройки по умолчанию. Личное правило может разрешить доступ, закрытый для всей роли. «Наследовать» убирает выбранное исключение.</p><p className="small secondary">Доступ открывает раздел и его просмотр. Видимость сотрудников и возможность изменять данные определяются рабочей ролью. Профиль, пароль и уведомления доступны всегда. Управление правами и аудит доступны администраторам. Сессии и устройства доступны только разработчику и через эту матрицу не назначаются.</p></Card>
