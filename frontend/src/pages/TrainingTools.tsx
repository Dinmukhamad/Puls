...
export function TrainerHome() {
  ...
  return <div className="stack"><header className="page-head"><div><h1 className="page-title">Кабинет тренера</h1><p className="page-subtitle">{user?.full_name} · обучение всех операторов</p></div></header>
    ...
    <Card title="Проверка перед публикацией"><p>Кнопка «Пройти как оператор» открывает тестовое прохождение. Оно не входит в рабочую статистику и не выдаёт коины.</p><div className="training-home-links"><Link to="/admin/learning?kind=simulator">Открыть сценарии Driver Simulator →</Link></div></Card>
  </div>;
}
...
export function TrainingAnalyticsPage() {
  ...
  return <div className="stack"><nav className="segmented driver-analytics-tabs" aria-label="Раздел аналитики">
    <Link className={`segmented__item${driverView ? "" : " is-active"}`} aria-current={!driverView ? "page" : undefined} to="/admin/learning-analytics">Прохождение материалов</Link>
    <Link className={`segmented__item${driverView ? " is-active" : ""}`} aria-current={driverView ? "page" : undefined} to="/admin/learning-analytics?view=driver">Driver Simulator</Link>
...
function LearningAnalyticsReport() {
  ...
  return <div className="stack"><header className="page-head"><div><h1 className="page-title">Аналитика обучения</h1><p className="page-subtitle">Только результаты операторов. Тестовые прохождения сотрудников исключены.</p></div><Button disabled={exporting.isPending || !data} onClick={() => exporting.mutate()}>Скачать CSV</Button></header>
    <Card title="Фильтры"><div className="training-analytics-filters">
      <label className="field"><span className="field__label">С даты</span><input className="input" type="date" value={params.get("date_from") ?? ""} onChange={e => change("date_from", e.target.value)} /></label>
      <label className="field"><span className="field__label">По дату</span><input className="input" type="date" value={params.get("date_to") ?? ""} onChange={e => change("date_to", e.target.value)} /></label>
      <label className="field"><span className="field__label">Поиск оператора</span><input className="input" value={search} onChange={e => setSearch(e.target.value)} /></label>
      <label className="field"><span className="field__label">Оператор</span><select className="input" value={params.get("user_id") ?? ""} onChange={e => change("user_id", e.target.value)}><option value="">Все операторы</option>{params.get("user_id") && !people.data?.items.some(x => String(x.id) === params.get("user_id")) && <option value={params.get("user_id")!}>Выбранный оператор</option>}{people.data?.items.map(x => <option value={x.id} key={x.id}>{x.full_name}</option>)}</select></label>
      <label className="field"><span className="field__label">Тип</span><select className="input" value={learningKindFilter(params.get("kind")) === "all" ? "" : "simulator"} onChange={e => change("kind", e.target.value)}><option value="">Все типы</option>{VISIBLE_LEARNING_KINDS.map(key => [key, LEARNING_LABELS[key]]).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
      <label className="field"><span className="field__label">Материал обучения</span><select className="input" value={params.get("content_id") ?? ""} onChange={e => change("content_id", e.target.value)}><option value="">Все материалы</option>{contents.data?.map(x => <option value={x.id} key={x.id}>{x.title}</option>)}</select></label>
      <label className="field"><span className="field__label">Статус</span><select className="input" value={params.get("state") ?? ""} onChange={e => change("state", e.target.value)}><option value="">Все статусы</option>{Object.entries(STATES).map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select></label><Button onClick={() => { setParams({}); setSearch(""); }}>Сбросить</Button>
