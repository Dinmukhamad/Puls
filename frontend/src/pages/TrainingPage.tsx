...
export function TrainingPage() {
  ...
  return <div className="stack training-page">
    ...
    {query.data && <>
      ...
      <div className="training-filters"><label className="field"><span className="field__label">Раздел</span><select className="input" value={kind} onChange={(e) => update("kind", e.target.value)}><option value="all">Всё обучение</option>{VISIBLE_LEARNING_KINDS.map(key => [key, LEARNING_LABELS[key]]).map(([key, value]) => <option key={key} value={key}>{value}</option>)}</select></label><SegmentedControl label="Статус обучения" value={state} options={[{value:"all", label:"Все"},{value:"new",label:"Новые"},{value:"in_progress",label:"В процессе"},{value:"passed",label:"Пройденные"}]} onChange={(value) => update("state", value)} /></div>
