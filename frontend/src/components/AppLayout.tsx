...
import { PwaUpdateDot } from "../pwa/PwaProvider";
  ...
  return <div className={collapsed ? "shell shell--collapsed" : "shell"}>
    <GlassSurface as="aside" variant="regular" className="sidebar">
      ...
      <nav className="sidebar__nav" aria-label="Основные разделы">
        {items.map((item) => <Link key={item.id} to={item.to} className={active?.id === item.id ? "nav-item is-active" : "nav-item"} aria-current={active?.id === item.id ? "page" : undefined} title={collapsed ? item.label : undefined} aria-label={collapsed ? item.label : undefined}><span className="nav-item__icon pwa-update-anchor"><item.icon size={20} />{item.id === "profile" && <PwaUpdateDot />}</span><span className="nav-item__label">{item.label}</span></Link>)}
      </nav>
      {user && <Link to="/profile" className="sidebar__user" aria-label={`Профиль: ${user.full_name}`}><span className="pwa-update-anchor"><Avatar name={user.full_name} id={user.id} size={36} /><PwaUpdateDot /></span><span className="sidebar__user-text"><span className="sidebar__user-name">{user.full_name}</span><span className="sidebar__user-role">{ROLE_LABELS[user.role]}{user.group ? ` · ${user.group.name}` : ""}</span></span></Link>}
    </GlassSurface>
    ...
    <GlassSurface as="nav" variant="prominent" className={compactTabBar ? "tabbar tabbar--compact" : "tabbar"} aria-label="Основные разделы, мобильное меню">
      {primary.map((item) => <Link key={item.id} to={item.to} aria-label={item.label} aria-current={active?.id === item.id ? "page" : undefined} className={active?.id === item.id ? "tab is-active" : "tab"}><span className="tab__icon pwa-update-anchor"><item.icon size={22} />{item.id === "profile" && <PwaUpdateDot />}</span><span className="tab__label">{item.label}</span></Link>)}
      {overflow.length > 0 && <button type="button" className={menuOpen || overflowActive ? "tab is-active" : "tab"} aria-label="Ещё разделы" aria-haspopup="dialog" aria-expanded={menuOpen} aria-controls={menuOpen ? menuId : undefined} onClick={() => setMenuOpen(true)}><span className="tab__icon pwa-update-anchor"><MenuIcon size={22} />{!primary.some((item) => item.id === "profile") && <PwaUpdateDot />}</span><span className="tab__label">Ещё</span></button>}
    </GlassSurface>
    {menuOpen && <Sheet id={menuId} title="Разделы Puls" onClose={closeMenu} size="s"><nav className="mobile-menu" aria-label="Все основные разделы">{items.map((item) => <Link key={item.id} to={item.to} className={active?.id === item.id ? "nav-item is-active" : "nav-item"} aria-current={active?.id === item.id ? "page" : undefined} onClick={closeMenu}><span className="nav-item__icon pwa-update-anchor"><item.icon size={20} />{item.id === "profile" && <PwaUpdateDot />}</span><span>{item.label}</span></Link>)}</nav>{separateProfile && <Link to={ACCOUNT_SECTION.to} className="section-navigation__account" onClick={closeMenu}><span className="pwa-update-anchor">{user && <Avatar name={user.full_name} id={user.id} size={32} />}<PwaUpdateDot /></span><span>Мой профиль и настройки</span></Link>}</Sheet>}
  </div>;
}
