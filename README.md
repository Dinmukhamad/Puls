...
```bash
ruff check app tests    # линт (конфиг в pyproject.toml)
...
В production также обязательны `AUTO_CREATE_TABLES=false`, `ENABLE_DEMO_DATA=false`,
`AUTH_COOKIE_SECURE=true` — иначе `check_production_safety()` не даст приложению стартовать.
...
- **ruff** — единственный линтер (`pyproject.toml`: E/F/I/UP/B, line-length 100,
  E501 в legacy-коде осознанно игнорируется). Запуск: `ruff check app tests`.
...
- **CI** — `.github/workflows/checks.yml`: ruff, compileall, pytest,
  `alembic upgrade head` на PostgreSQL 17 (service-container), сборка
  фронтенда и проверка, что закоммиченные бандлы синхронны с исходниками.

---

## Аналитика: отсутствие данных и «Здоровье команды»

Отсутствующее значение и ноль — разные вещи, и код обязан их различать.

- Качество оценивается **выборочно**. Оператор без единой проверки за период
  имеет `quality_avg = null`, а не 0. То же для КВЗ и эффективности при
  `base_hours = 0`.
- Индекс здоровья оператора (`_operator_health_snapshot`) — среднее
  нормированных компонентов (100 = цель) **только по измеренным** показателям.
  Неизмеренный компонент из среднего исключается, а не входит в него нулём.
  Состав индекса отдаётся в `health_components`, по команде — в
  `team_health.components`.
- Пропуск данных даёт статус `watch` (нужно внимание — данных нет), а не
  `critical` (подтверждённо плохой показатель). В `priority_score` вес
  пропуска прежний, поэтому такие операторы по-прежнему в начале списка.
- Штрафы известны всегда: ноль штрафов — реальный ноль, компонент считается.

Норма часов кешируется в пределах сессии SQLAlchemy (= одного HTTP-запроса):
аналитика запрашивает её для каждого оператора за каждый месяц периода, и без
кеша это давало по одному SELECT на оператора. Кеш живёт не дольше запроса,
записи в справочник норм сбрасывают его явно.

## Запросы к БД на списочных эндпоинтах

Списки собирают ответ по коллекции сущностей, поэтому легко получают N+1:
на тестовых данных он незаметен, а на проде превращается в сотни запросов.

Правила:

- Связанные сущности загружаются пачкой до цикла, а карта передаётся
  в сборщик строки **явным аргументом**. Полагаться на identity map сессии
  нельзя: она держит слабые ссылки, и без сохранённого результата объекты
  успевает собрать сборщик мусора — запрос уходит снова на каждую строку.
- Справочники (правила коинов, активные уровни, нормы часов) читаются один раз
  на запрос и передаются вниз параметром.
- `operator_level_badge` не должен звать `operator_level_summary`: значку не
  нужны ни разрывы до следующего уровня, ни строки наград.

Регрессию сторожит `tests/test_query_budget.py`: он измеряет не абсолютное
число запросов, а его прирост при добавлении операторов.
...
GET  /api/analytics/dashboard            Экран руководителя одним ответом
GET  /api/analytics/summary              KPI аналитики
GET  /api/analytics/operators            Таблица операторов (пагинация + сортировка)
GET  /api/analytics/daily-dynamics       Посуточная динамика метрики
GET  /api/analytics/available-periods    Доступные периоды
GET  /api/analytics/groups-list          Справочник групп в области доступа
GET  /api/analytics/glossary             Определения и цели показателей
GET  /api/analytics/export.xlsx          Выгрузка таблицы операторов
...
app/
  ...
  modules/
    ...
    wheel/
    work_norms/
  services/              # общие сервисы: seed, миграции схемы, кеши
```

New code should be added under `app/modules/<domain>/`.

Domain schemas live next to the owning module in `app/modules/<domain>/schemas.py`.
...
(их синхронность с исходниками проверяет CI):
...
`css/tokens.css` пишется вручную и сборщиком не генерируется.
