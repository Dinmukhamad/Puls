...
def login_base(row: OperatorRow, fallback_index: int) -> str:
    ...
    if len(parts) >= 2:
        return f"group51_{parts[0]}_{parts[1]}"
    if parts:
        return f"group51_{parts[0]}"
    return f"group51_operator_{fallback_index:03d}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bulk import Group 51 operators from xlsx.")
    parser.add_argument("--file", required=True, help="Path to operators.xlsx")
    parser.add_argument("--group", default=GROUP_NAME_DEFAULT, help="Target group name")
    parser.add_argument("--apply", action="store_true", help="Write changes to the database")
    parser.add_argument("--actor-user-id", type=int, default=None, help="User id for audit log records")
    parser.add_argument("--credentials-output", default="", help="CSV path for one-time credentials")
    parser.add_argument("--report-output", default="", help="JSON path for import report")
    parser.add_argument("--allow-default-db", action="store_true", help="Allow running without DATABASE_URL")
...
def main() -> int:
    ...
    credentials_path = Path(args.credentials_output) if args.credentials_output else root / "secure_outputs" / f"operator_credentials_group51_{now_stamp}.csv"
    report_path = Path(args.report_output) if args.report_output else root / "secure_outputs" / f"operator_import_report_group51_{now_stamp}.json"
    ...
    try:
        ...
        for fallback_index, row in enumerate(rows, start=1):
            ...
            if operator is None:
                created_operators += 1
                username = unique_login(login_base(row, fallback_index), used_usernames)
